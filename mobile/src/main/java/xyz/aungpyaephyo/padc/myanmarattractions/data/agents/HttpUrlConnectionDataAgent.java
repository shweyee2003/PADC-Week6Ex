        ...
        new AsyncTask<Void, Void, List<AttractionVO>>() {
            ...
            protected List<AttractionVO> doInBackground(Void... voids) {
                ...
                try {
                    ...
                    HttpURLConnection connection = (HttpURLConnection) url.openConnection();//2.connect url

                    // just want to do an HTTP POST here
                    connection.setRequestMethod("POST");//3.define http method

                    // uncomment this if you want to write output to this url
                    //connection.setDoOutput(true);

                    // give it 15 seconds to respond
                    connection.setReadTimeout(15 * 1000);//4. define timeout pass with milisecond (1000 to get sec)

                    connection.setDoInput(true);//5.need to pass  parameter must define
                    connection.setDoOutput(true);//5.need to pass  parameter must define

                    //put the request parameter into NameValuePair list.
                    List<NameValuePair> params = new ArrayList<>();//6.creae list to pass parameter to api
                    params.add(new BasicNameValuePair(MyanmarAttractionsConstants.PARAM_ACCESS_TOKEN, MyanmarAttractionsConstants.ACCESS_TOKEN));//pass param to api

                    //step6
                    //write the parameters from NameValuePair list into connection obj.
                    OutputStream outputStream = connection.getOutputStream();//use jave IO package
                    BufferedWriter writer = new BufferedWriter(
                            new OutputStreamWriter(outputStream, "UTF-8"));//use jave IO package
                    writer.write(getQuery(params));
                    writer.flush();
                    writer.close();
                    outputStream.close();

                    connection.connect();//7.connect url obj

                    //step 8
                    // read the output from the server
                    reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));//create readerobj to read response
                    ...
                    String responseString = stringBuilder.toString();//9.put stringobj from stringbuilder
                    AttractionListResponse response = CommonInstances.getGsonInstance().fromJson(responseString, AttractionListResponse.class);//pass parma: data string,class that want to show
                    List<AttractionVO> attractionList = response.getAttractionList();

                    return attractionList;//return to onPostExecute
